...
from data_processing import (
    ...
    load_time_series  # Make sure this is defined in data_processing
)
from plotting import (
    plot_slice,
    plot_highlighted_slice,
    plot_3d_brain,
    annotate_slice,
    generate_charts,
    plot_time_series
...
with st.sidebar.expander("3D Scan"):
    ...
    try:
        time_series = load_time_series(uploaded_csv)
    ...
    if uploaded_csv is not None and 'time_series' in locals():
        if len(data.shape) == 4:
            ...
            with st.expander("GLM Results"):
                ...
                st.write(
                    f"The General Linear Model (GLM) analysis for the selected region '{atlas_labels[region_index_val]}' "
